...
\begin{center}
\begin{quantikz}
    \lstick{$\ket{0^n}$} & \gate[1]{QFT_M} & \gate[2]{f(a) = x^{a} \pmod{N}} & \gate[1]{QFT_M} & \meter{} \\
    \lstick{$\ket{0^n}$} & \qw & \qw
\end{quantikz}
\end{center}
...
In the general case, $r$ does not divide $M$. But with constant probability, we see $\ell$ such that $|\ell r \mod M| \leq r/2$.
In other words,
