...
\begin{theorem}
    ...
    be an exact sequence of modules. The following are equivalent:
    \begin{enumerate}
        \item There exists a morphism $\varphi: M'' \to M$ such that $g \circ \varphi = \text{id}_{M''}$.
        \item There exists a morphism $\psi: M \to M'$ such that $\psi \circ f = \text{id}_{M'}$
    \end{enumerate}
    If either is satisfied, then 
    \[ M = \textrm{Im} (f) \oplus \textrm{Ker} (\psi) = \textrm{Im}(\varphi) \oplus \textrm{Im}(g) = M' \oplus M'' \]
...
where $M \cong N \oplus \ker \phi$. We claim this is true if and only if
