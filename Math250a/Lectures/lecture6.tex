...
\end{theorem}
Notice that the action of conjugation by some element $g \in G$ is an automorphism from $G$ to itself,
...
These theorems are all called isomorphism theorems.
Note that there is a surjective canonical map $\phi: G \to G/H$ for normal $H$
that sends each element $g \mapsto gH$.
\begin{enumerate}
    \item Let $f: G \to G'$ be a homomorphism. Then there exists a bijective homomorphism induced by $f$ as $G/\ker f \to \textrm{im} f$.
    \item Let $H < G$ and let $N$ be the intersection of all normal subgroups of $H$. Then there is canonical $G \to G/N$
    that we can factor homomorphisms through.
    \item $(G/H)/(H/K) \cong G/H$.
    \item $H/(H \cap K) \cong HK/K$.
    \item Inverse images (under a group homomorphism) of normal subgroups are normal.
\end{enumerate}

\begin{definition}
    A group is said to be \textbf{solvable} if it has an abelian tower (or cyclic WLOG), e.g.
    \[ G = G_0 \supset G_1 \supset \dots \supset G_m = \{e\} \]
    where $G_i/G_{i + 1}$ is abelian (or cyclic) group.
\end{definition}
...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        \item The number of Sylow $p$-subgroups divides the index of the Sylow $p$-groups.
        \item For any Sylow $p$-subgroup $P$, the number of Sylow $P$ subgroups is $[G : N_G(P)]$.
    \end{enumerate}
\end{theorem}