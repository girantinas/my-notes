...
Let's recap last time. Suppose $M$ is a finitely-generated torsion module over a PID $R$; we wish to show
$M \cong \bigoplus_a R/(a)$ for some $a$.
We saw last time that
\[ M \cong \bigoplus_{p \text{prime}} M(p) \]
where $M(p) = \{ m \in M \mid p^n m = 0 \text{for some } n \}$. Thus, without loss of generality, we can just take
...
\[\begin{bmatrix}
  ...
  1 &  \lambda_1 
\end{bmatrix}\]
Means $(x- \lambda_1)^2$. For complex eigenvalues $\lambda_k = a_k + ib_k$
(say of multiplicity 2) you get
\[\begin{bmatrix}
    C_k & 0 \\
    I & C_k
